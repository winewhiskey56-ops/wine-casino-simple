...
# --- ОТРЕНДЕРИТЬ ИНТЕРФЕЙС В ЗАВИСИМОСТИ ОТ СТАТУСА ВХОДА ---
if not st.session_state.authenticated:
    ...
    # --- ЭКРАН АВТОРИЗОВАННОГО ПОЛЬЗОВАТЕЛЯ (ИСПРАВЛЕНО: ТЕПЕРЬ ОТОБРАЖАЕТСЯ ПОСЕРЕДИНЕ) ---
    
    # Проверка аварийной сессии при самом первом входе в сессию игры
    ...
    # --- ЛОГИКА ОТОБРАЖЕНИЯ СТРАНИЦ АДМИНА ИЛИ ВЕДУЩЕГО ---
    ...
    # Интерфейс обычного ведущего (Главное меню)
        ...
        for p in st.session_state.players:
            ...
            if abs(float(b_alc) - float(target["alc"])) <= 0.5: 
                win_coef += 2
        ...
        if col_r1.button("⬅️ Назад к ставкам"):
            st.session_state.page = "bets_page"
            st.rerun()
    ...
    # --- ПРАВКА 2: ПОДВАЛ ОБРАТНОЙ СВЯЗИ (ПЕРЕНЕСЕН ВНУТРЬ СЕССИИ, БЕЗ НОМЕРА ТЕЛЕФОНА) ---
    st.markdown("---")
    with st.expander("💬 Связь с разработчиком"):
        st.write("Вы можете сказать спасибо и поддержать улучшение системы:")
        st.code("Реквизиты Т-Банк (Карта): [Перевод по номеру карты в приложении]", language="text")
